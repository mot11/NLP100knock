...
def format_url(url_string):
    p = re.compile(' +')
    output = p.sub('%20', url_string)
    return output


def get_from_web(url_string):
...
def main():
    ...
    url = ('https://commons.wikimedia.org/w/api.php?action=query&titles=File:' + output['国旗画像'] +
           '&prop=imageinfo&&iiprop=url&format=json')
    url = format_url(url)
    data = get_from_web(url)
    real_url = extract_url(data)
    print('\n国旗画像のurl:\n\t', real_url)
